...
function buildStrictFallbackAiImageUrl(prop: {
  ...
  const strictPrompt = [buildStrictRealEstatePrompt(prop), prop.variant || ''].filter(Boolean).join(', ');
  const encoded = encodeURIComponent(strictPrompt);
  const variantSlug = String(prop.variant || 'base').replace(/[^a-z0-9]+/gi, '-').toLowerCase();
  return `https://image.pollinations.ai/prompt/${encoded}?width=1024&height=640&seed=${encodeURIComponent(`miami-strict-${prop.id}-${variantSlug}`)}&model=flux&nologo=true`;
}
...
export async function enrichMiamiListingsWithAIGeneratedPhotos(
  ...
  for (const prop of properties) {
    ...
    if (hasNonAiImages && !replaceExisting) {
      skippedWithImages++;
      continue;
    }
...
export async function resetMiamiListingsPhotosWithAIGenerated(limit = 500): Promise<{
  processed: number;
  imagesAdded: number;
  skippedWithImages: number;
}> {
